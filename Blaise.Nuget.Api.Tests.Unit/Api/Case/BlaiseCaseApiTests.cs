...
namespace Blaise.Nuget.Api.Tests.Unit.Api.Case
{
    public class BlaiseCaseApiTests
    {
        ...
        [Test]
        public void Given_Valid_Arguments_When_I_Call_GetLiveDate_Then_The_Correct_Service_Method_Is_Called()
        {
            //arrange
            _caseServiceMock.Setup(d => d.GetLiveDate(_connectionModel, It.IsAny<string>(), It.IsAny<string>()));

            //act
            _sut.GetLiveDate(_instrumentName, _serverParkName);

            //assert
            _caseServiceMock.Verify(v => v.GetLiveDate(_connectionModel, _instrumentName, _serverParkName), Times.Once);
        }
